...
#'
#'@param val \code{shift()} iterates along the elements of \code{val} to create
#'  shifts.
...
#'@examples
#'shift(rep(1:20), bases = c(rep(4.5,8), rep(NA,12)), trigger = 9)
...
shift <- function(val, base, base_ext, trigger = 6) {
  ...
  for (gap in gaps) {
    ...
    indices <- shift_ticker(base_ext[gap[1]], val[gap]) + gap[1] - 1
