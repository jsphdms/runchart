...
test_that("rc_fields() handles basic cases", {
  expect_equal(
    rc_fields(rep(0, 8)),
...
test_that("rc_fields() handles corner cases", {

  # shift then sus
  expect_equal(
    rc_fields(c(rep(0, 8), rep(1, 6), rep(-1, 9))),
  ...
  # sus then shift
  expect_equal(
    rc_fields(c(rep(0, 8), rep(1, 9), rep(-1, 6))),
  ...
  # almost shift
  expect_equal(
    rc_fields(c(rep(0, 8), rep(1, 4), 0, NA, 1, 0, NA)),
  ...
  # delayed shift
  expect_equal(
    rc_fields(c(rep(0, 8), rep(1, 4), 0, NA, 1, 0, NA, 1)),
    data.frame(base = c(rep(0, 8), rep(NA, 10)),
               base_ext = rep(0, 18),
               base_label = c(0, rep(NA, 17)),
               shift = c(rep(NA, 8), rep(1, 4), 0, NA, 1, 0, NA, 1),
               val = c(rep(0, 8), rep(1, 4), 0, NA, 1, 0, NA, 1))
  )

  # delayed sus
  expect_equal(
    rc_fields(c(rep(0, 8), rep(1, 7), 0, NA, 1, 0, NA, 1)),
  ...
  # almost sus
  expect_equal(
    rc_fields(c(rep(0, 8), rep(1, 7), 0, NA, 1, 0, NA)),
  ...
  # consecutive sus
  expect_equal(
    rc_fields(c(rep(0, 8), rep(1, 9), rep(-1, 9))),
  ...
  # consecutive shift
  expect_equal(
    rc_fields(c(rep(0, 8), rep(1, 7), rep(-1, 6))),
...
test_that("rc_fields() handles NAs", {

  # all NA
  expect_equal(
    rc_fields(rep(NA_real_, 10)),
  ...
  expect_equal(
    rc_fields(c(rep(0, 8), rep(1, 6), NA)),
