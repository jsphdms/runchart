...
test_that("basic_shift() handles corner cases", {
  ...
  expect_equal(basic_shift(base = 0, val = c(rep(1, 5), rep(0, 2), 1)), 1:8)
  expect_equal(basic_shift(base = 0, val = c(rep(1, 5), rep(NA, 2), 1)),
               c(1:5, 8))
  expect_equal(basic_shift(base = 0, val = c(1:5, NA, 0, -1:-5, 0, NA)), NULL)
  expect_equal(basic_shift(base = 0, val = c(1:5, NA, 0, 1)), c(1:5, 7, 8))
  ...
  expect_equal(basic_shift(base = 0, val = c(rep(1, 4), rep(NA, 4), rep(1, 2))),
               c(1:4, 9:10))
  expect_equal(basic_shift(base = 0, val = c(rep(1, 4), rep(0, 4), rep(1, 2))),
               1:10)
  expect_equal(basic_shift(base = 0, val = c(rep(c(1, NA), 5), 1)),
               seq(1, 11, by = 2))
